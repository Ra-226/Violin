...
if __name__ == '__main__':
    ...
    queryRate = 0.25
    df = pd.DataFrame(0.0, range(2 * len(r)),
                      ['count', "r", "ShieldAlpha", "attack", "acc", "len", "size", "time", "setup_Overhead",
                       "inj_Overhead"])
    flag = 0
    delta = 3000
    word_len = 1000
    ...
    for v_x in r:
        ...
        for times in range(run_times):
            ...
            Violin_acc, Violin_result, Violin_total_inject_size, Violin_total_inject_length, Violin_time, setup_Overhead, inj_Overhead = Violin.Violin_set_main(
                ...
                word_size_set, group, v_x)

            df.loc[flag] = [times, str(int(v_x)), Alpha, "Violin", Violin_acc, Violin_total_inject_length,
                            Violin_total_inject_size, Violin_time, setup_Overhead, inj_Overhead]
            flag += 1

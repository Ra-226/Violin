...
class Threshold:
    ...
    def BVA_inject(self):
        ...
        for s in size_each_doc:
            if self.T >= s:
                ...
                self.total_inject_length += math.ceil(len(self.chosen_kws) * 0.5 / self.T) * (
                            math.floor(s / self.T) + last_part_size)
                # 分割重填前后size不变
        F = []
    ...
    def Violin_inject(self):
        ...
        self.total_inject_size = (self.T - self.total_inject_length) * self.total_inject_length + (
                    self.total_inject_length - 1) * self.total_inject_length / 2

...
def polt_figure(df):
    ...
    bar = sns.barplot(data=df, x="x", y="acc", hue='attack', hue_order=["BVMA", "BVA-l", "BVA-h", "Decoding", "Violin"],
                      ...
                      palette=[color_def[0], color_def[3], color_def[2], color_def[1], color_def[4]])
...
def polt_figure_size(df, str):
    ...
    if str == "len":
        sns.barplot(data=df, x="attack", y=str, hue='attack',
                    hue_order=["BVMA", "BVA-l", "BVA-h", "Decoding", "Violin"],
...
if __name__ == '__main__':
    ...
    for v_x in x:
        ...
        for times in range(run_times):
            ...
            Decoding_acc, Decoding_total_inject_size, Decoding_total_inject_length, offset, Decoding_time = Decoding.Decoding_main(
                wordSet, query, wordAccess, total_size)
            df.loc[flag] = [times, v_x, "Decoding", Decoding_acc, Decoding_total_inject_length,
                            Decoding_total_inject_size, Decoding_time]
            flag += 1
            ...
            BVMA_acc_temp, BVMA_total_inject_size, BVMA_total_inject_length, BVMA_time = BVMA.BVMA_NoSP_main(wordSet,
                                                                                                             ...
                                                                                                             wordAccess,
                                                                                                             total_size)
            df.loc[flag] = [times, v_x, "BVMA", BVMA_acc_temp, BVMA_total_inject_length, BVMA_total_inject_size,
                            BVMA_time]
            flag += 1
            ...
            df.loc[flag] = [times, v_x, "BVA-l", low_BVA_acc_temp, low_BVA_total_inject_length,
                            low_BVA_total_inject_size, low_BVA_time]
            flag += 1

            gamma = offset // 4
            high_BVA_acc_temp, high_BVA_total_inject_size, high_BVA_total_inject_length, high_BVA_time = BVA.BVA_main(
                wordSet, query, wordAccess, total_size, gamma)
            df.loc[flag] = [times, v_x, "BVA-h", high_BVA_acc_temp, high_BVA_total_inject_length,
                            high_BVA_total_inject_size, high_BVA_time]
            flag += 1

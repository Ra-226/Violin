# -*- coding: utf-8 -*-
"""
Created on Sat Aug 24 21:08:34 2024
...
def polt_figure(df):
    ...
    sns.barplot(data=df,x="ShieldAlpha", y="acc",hue='attack',hue_order=["BVMA","BVA","Violin"],legend=True, palette=[color_def[0],color_def[3],color_def[4]])
    ...
    plt.tick_params()
    plt.savefig("./pic/" + 'padding_ShileldDB_Movies.pdf', bbox_inches = 'tight', dpi = 600)#.pdf
    plt.show()
...
def polt_figure_size(df):
    ...
    plt.ylabel('Overhead (\%)')
    plt.tick_params()
    plt.savefig("./pic/" + 'padding_ShileldDB_{}_Movies.pdf'.format("overhead"), bbox_inches = 'tight', dpi = 600)#.pdf 
    
    plt.show()

if __name__=='__main__':
    file = "./pkl/padding_ShileldDB_Movies.pkl"
    ...
    if os.path.exists(file):
        ...
        cnt = 0
        for i in df.index:
            Overhead.loc[cnt] = [df.loc[i]["attack"], df.loc[i]["ShieldAlpha"], "Setup\&Fill", (df.loc[i]["setup_Overhead"] - 1) * 100]
            Overhead.loc[cnt + 1] = [df.loc[i]["attack"], df.loc[i]["ShieldAlpha"], "Inj\&Fill", (df.loc[i]["inj_Overhead"] - 1) * 100]
            cnt += 2
        polt_figure(df)
    ...
    with open('./Datasets/Movies_vol_access.pkl','rb') as f:
        pkl=pickle.load(f)
    with open('Datasets/doc_size_movies.pkl', 'rb') as f:
    ...
    df = pd.DataFrame(0.0,range(2*len(ShieldAlpha)),['count',"ShieldAlpha","attack","acc","len","size","time", "setup_Overhead", "inj_Overhead"])
    flag = 0
    delta = 3000
    word_len = 1000
    run_times = 10
    ...
    for v_x in ShieldAlpha:
        ...
        for times in range(run_times):
            ...
            Violin_acc, Violin_result, Violin_total_inject_size, Violin_total_inject_length,Violin_time, setup_Overhead, inj_Overhead = Violin.Violin_set_main(
                wordSet, list(pkl[0].keys()), query, total_size, real_length, min_file_size, max_file_size, avg_file_size, word_size_set, group)
            ...
            BVMA_acc, BVMA_total_inject_size, BVMA_total_inject_length,BVMA_time, setup_Overhead, inj_Overhead = BVMA.BVMA_NoSP_main(
                wordSet, list(pkl[0].keys()), query, total_size, real_length, min_file_size, max_file_size, avg_file_size, group)
            ...
            BVA_acc, BVA_total_inject_size, BVA_total_inject_length,BVA_time, setup_Overhead, inj_Overhead = BVA.BVA_main(
                wordSet, list(pkl[0].keys()), query, total_size, real_length, gamma, min_file_size, max_file_size, avg_file_size, group)
            df.loc[flag] = [times, v_x, "BVA", BVA_acc,BVA_total_inject_length,BVA_total_inject_size,BVA_time, setup_Overhead, inj_Overhead]
            flag += 1

    with open("./pkl/padding_ShileldDB_Movies.pkl","wb") as f:
        pickle.dump(df,f)

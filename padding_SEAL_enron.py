    ...
    for v_x in x:
        ...
        for times in range(run_times):
            ...
            Violin_acc, Violin_result, Violin_total_inject_size, Violin_total_inject_length, Violin_time = Violin.Violin_set_main(
                wordSet, query, wordAccess, total_size, word_size_set, real_length, seal_x, min_file_size,
                max_file_size)
            df.loc[flag] = [times, v_x, "Violin", Violin_acc, Violin_total_inject_length, Violin_total_inject_size,
                            Violin_time]
            flag += 1

            BVMA_acc, BVMA_total_inject_size, BVMA_total_inject_length, BVMA_time = BVMA.BVMA_NoSP_main(wordSet, query,
                                                                                                        wordAccess,
                                                                                                        total_size,
                                                                                                        real_length,
                                                                                                        seal_x,
            ...
            BVA_acc, BVA_total_inject_size, BVA_total_inject_length, BVA_time = BVA.BVA_main(wordSet, query, wordAccess,

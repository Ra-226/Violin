...
def get_size_and_length_after_injection_padding(word_set, size_after_setup_padding, 
    ...
    for Gp in group:
        ...
        for i in Gp:
            ...
            if max_length_of_each_cluster - injection_length[k]>20:
                ...
                size_set_after_injection_padding[k] += [random_size] * (max_length_of_each_cluster - injection_length[k])
            else:
    ...
    return pd.Series(size_after_injection_padding), pd.Series(length_after_injection_padding), size_set_after_injection_padding, total_size_after_padding / total_size_no_padding
...
def Violin_set_recover(wordSet, word_set, query, group, min_file_size, max_file_size,
                     total_size, length_after_setup_padding, word_size_set):
    ...
    size_after_injection_padding, length_after_injection_padding, size_set_after_injection_padding, inj_Overhead = get_size_and_length_after_injection_padding(word_set, total_size, 
                                                    length_after_setup_padding, group, 
...
def Violin_set_main(wordSet, word_set, query, total_size, real_length, 
                  min_file_size, max_file_size, word_size_set, group):
    ...
    acc, result, total_inject_size, total_inject_length, inj_Overhead = Violin_set_recover(
        ...
        size_after_setup_padding, length_after_setup_padding, size_set_after_setup_padding)
    runtime = time.time() - s
